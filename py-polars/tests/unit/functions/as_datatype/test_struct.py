...
# parametric tuples: (expr, is_scalar, values)
agg_expressions = [
    (pl.lit(1, pl.Int64), True, [1, 1, 1]),  # LiteralScalar
    (pl.col("n"), False, [1, 2, 3]),  # NotAggregated
    (pl.int_range(1, pl.len() + 1), False, [1, 2, 3]),  # AggregatedList
...
def test_struct_schema_in_group_by_apply_expr_24168(
    ...
    if n_rows in [0, 1, 2]:
        # note: type hack for when n_rows = 0 using head()
        structs = [{"lhs": lhs[2][i], "rhs": rhs[2][i]} for i in range(2)]
        struct_col_scalar = [structs[0]]
        struct_col_non_scalar = [structs[: max(1, n_rows)]] 
        struct_col = struct_col_scalar if lhs[1] and rhs[1] else struct_col_non_scalar
        expected = pl.DataFrame({"g": [10], "expr": struct_col}).head(n_rows)
